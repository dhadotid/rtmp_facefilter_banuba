...
import com.pedro.rtplibrary.Constant;
...
//  private Camera2ApiManager cameraManager;
  ...
  public BanubaSdkManager banubaSdkManager;
  ...
  protected long startStreamTimeStamp;
...
//    cameraManager = new Camera2ApiManager(context);
  ...
  private IEventCallback banubaSdkEventCallback = new IEventCallback() {
    @Override
    ...
    @Override
    public void onVideoRecordingStatusChange(boolean b) {
    ...
    public void onFrameRendered(@NonNull Data data, int width, int height) {
      ...
      byte[] nv21 = YUVUtil.ARGBtoYUV420SemiPlanar(input, width, height);
  ...
  public boolean prepareVideo(int width, int height, int bitrate) {
    return prepareVideo(width, height, 30, bitrate, false, 2, 0);
  }
...
//        cameraManager.closeCamera();
        ...
        banubaSdkManager.attachSurface(videoEncoder.getInputSurface());
...
//        cameraManager.prepareCamera(surfaceView.getHolder().getSurface());
        ...
        banubaSdkManager.attachSurface(new Surface(textureView.getSurfaceTexture()));
...
//        cameraManager.prepareCamera(glInterface.getSurfaceTexture(), width, height);
        banubaSdkManager.attachSurface(new Surface(glInterface.getSurfaceTexture()));
      }
...
//      cameraManager.openCameraFacing(cameraFacing);
  ...
  public void startPreview(int width, int height, int rotation){
    if (!isStreaming() && !onPreview && !isBackground) {
      previewWidth = width;
      previewHeight = height;
//      if (surfaceView != null) {
...
//      cameraManager.openLastCamera();
  ...
  public void startPreview() {
    startPreview(videoEncoder.getWidth(), videoEncoder.getHeight(), CameraHelper.getCameraOrientation(context));
  }

  public void startPreview(int width, int height){
    startPreview(width, height, CameraHelper.getCameraOrientation(context));
  }
...
//      cameraManager.closeCamera();
  ...
  public void startStream(String url) {
    streaming = true;
    startStreamTimeStamp = System.nanoTime() / 1000;
...
//      cameraManager.switchCamera();
      if (banubaSdkManager.getCameraFacing() == Facing.BACK) {
        banubaSdkManager.setCameraFacing(Facing.FRONT, true);
      } else {
        banubaSdkManager.setCameraFacing(Facing.BACK, false);
      }
    }
  }
...
//    }
  ...
  public void startForwardingFrames() {
    banubaSdkManager.getEffectPlayer().effectManager().setEffectVolume(0F);
    banubaSdkManager.getEffectPlayer().playbackPlay();
    banubaSdkManager.startForwardingFrames();
  }

  public void stopForwardingFrames() {
    banubaSdkManager.getEffectPlayer().playbackPause();
