...
//      Pattern.compile("^rtmps?://([^/:]+)(?::(\\d+))*/([^/]+)/?([^*]*)$");
  ...
  public boolean connect(String url) {
    ...
    String portStr = rtmpMatcher.group(3);
    port = portStr != null ? Integer.parseInt(portStr) : 443;//1935;
    appName = getAppName(rtmpMatcher.group(3), rtmpMatcher.group(4));
    streamName = getStreamName(rtmpMatcher.group(6));
